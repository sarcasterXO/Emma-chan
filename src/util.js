...
module.exports = class Util {
    ...
    static async pagination(msg, author, contents, init = true, currPage = 0) {
        if (init) for (const emoji of this.paginationEmojis) await msg.react(emoji).catch((e) => e.stack);
        ...
        collector
            .on("collect", (reaction) => {
                reaction.users.remove(author).catch((e) => e.stack);
                ...
                msg.edit(embed).catch((e) => e.stack);

                this.pagination(msg, author, contents, false, currPage);
            })
            .on("end", (_, reason) => {
                if (["time", "user"].includes(reason)) msg.reactions.removeAll().catch((e) => e.stack);
            });
    }
