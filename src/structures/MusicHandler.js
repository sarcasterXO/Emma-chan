...
module.exports = class MusicHandler {
    ...
    get node() {
        return this.client.manager.nodes.get("Emma");
    }
    ...
    async join(voice) {
        ...
        this.player
            .on("start", () => {
                ...
                if (this.textChannel) this.textChannel.send(util.embed().setTitle(`Now playing`).setDescription(`[${this.current.info.title}](${this.current.info.uri}) [${this.current.requester}]`))
                .then(m => m.delete({ timeout: this.current.info.length }).catch((err) => {null} ))
                .catch((e) => e.stack);
            })
            .on("end", (data) => {
                ...
                if (!this.queue.length) {
                    ...
                    if (this.textChannel) this.textChannel.send(util.embed().setDescription("Queue is empty. Leaving voice channel..")).catch((e) => e.stack);
                    this.reset();
                    return;
                }
                this.start();
