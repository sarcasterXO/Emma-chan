...
module.exports = {
    ...
    exec: async (msg, args) => {
        ...
        if (args.length) {
            ...
            return msg.channel.send(util.embed().setDescription("Missing args.")).catch((e) => console.log(e.stack));
        }
        ...
        try {
            ...
            const lyricsMsg = await msg.channel.send(embed).catch((e) => console.log(e.stack));
            if (splittedLyrics.length > 1) await util.pagination(lyricsMsg, msg.author, splittedLyrics);
        } catch (e) {
            if (e.message === "Sorry I couldn't find that song's lyrics") msg.channel.send(util.embed().setDescription(`${e.message}`)).catch((e) => console.log(e.stack));
            else msg.channel.send(`An error occured: ${e.message}.`).catch((e) => console.log(e.stack));   
        }
    }
};
